...
export async function getBookingsByUser(userId: string) {
  return prisma.booking.findMany({ where: { userId }, include: { passengers: true } });
}

export async function getBookingByPnr(userId: string, pnr: string) {
  const booking = await prisma.booking.findFirst({ where: { pnr, userId }, include: { passengers: true } });
...
export async function updateBookingStatus(userId: string, pnr: string, status: BookingStatus, transactionId?: string) {
  const now = new Date();
...
export async function updateBoardingPoint(userId: string, pnr: string, newBoardingStation: string) {
