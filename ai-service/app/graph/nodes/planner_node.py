...
async def planner_node(state: AgentState, claude_service: ClaudeService) -> Dict[str, Any]:
    """
    Understands user intent using the centralized ClaudeService wrapper.
    """
    raw_messages = state.get("messages", [])
    formatted_messages = _format_messages_for_claude(raw_messages)

    # Use structured response / low temperature for deterministic JSON output
    ...
    # Extract text from raw response content blocks
    raw_text = "".join(
    ...
    try:
        ...
        intent = data.get("intent", "general_chitchat")
