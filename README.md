...
*Built with FastAPI · LangGraph · OpenAI · MCP SDK · Next.js · MongoDB · PostgreSQL · Redis*


hey i see resuem turn you
...
beofre now book it saw nothing empty message form ai it i have process your request how can i help you further also in temrinal bfore book it i swa ocvo memort laoded log and itrace ise this reusem chat turn what isissu after gettign passneger it show all detial then aks can we prcceed with this athen confrimation itnerrupt but it happened after i queire now book it

Kiro
Let me read the websocket manager's full auto-resume and _run_graph flow carefully.
...
manager.py
Now I see the full picture. Let me trace through both bugs precisely.

The exact problem sequence:

The LangSmith trace shows resume: true as the input to a turn. This means on_query_send ran its auto-resume path (Command(resume=approved)). But after the booking completes and agent_node runs again (to produce the final "Your ticket is booked" reply), _extract_reply finds nothing because:

The AIMessage that agent_node produces after the tool result — that message is stored in the messages list in state. But _extract_reply scans for the last HumanMessage to find the boundary. Since Command(resume=True) never adds a HumanMessage to the graph state, last_human_idx points to the original booking request message, not the "yes proceed" message. Everything after that old HumanMessage is current-turn — that includes the old confirmation prompt AIMessage, ToolMessages, and the new success AIMessage. _extract_reply should find the success AIMessage...

Unless the agent_node cleanup on is_new_turn=True strips the ToolMessage. Let me check: after Command(resume) → human_approval → tool_executor → agent_node. At this point agent_loop_count in state. The booking interrupt happened when agent_loop_count was some value > 0. After resume, tool_executor_node doesn't change agent_loop_count. So agent_node sees prior_loop > 0, is_new_turn = False. ToolMessages are NOT stripped. ✅

The actual problem: the _extract_reply in the auto-resume path scans the final result["messages"] but the final AIMessage has empty content because agent_node ran post-tool and the reflection+route sent it to END before the model produced a text reply. Let me check agent_node behavior when loop_count > 1 and tool results are present — specifically whether is_new_turn=False but reset_fields={} means reflection_required is still True from the previous run, causing an immediate route to reflection_node before agent produces a text reply.

Actually — the real root cause is simpler. Look at the flow when the graph was interrupted:

agent_node loop=2, emits book_ticket tool call → pending_tool_calls=[book_ticket], agent_loop_count=2
→ human_approval_node → interrupt fires → graph suspends here mid-node
When Command(resume=True) comes in, human_approval_node finishes → returns {confirmed: True}. Then tool_executor_node runs → book_ticket succeeds → adds ToolMessage to messages, clears pending_tool_calls. Then agent_node runs with prior_loop=2, is_new_turn=False.

But here is the real bug: agent_loop_count is never reset between the interrupt and resume. After human_approval_node and tool_executor_node, agent_loop_count is still 2. Then agent_node runs → loop_count=3. It sees tool results in messages, produces the final success reply AIMessage, sets agent_loop_count=0, and routes to reflection or END. This should produce a valid reply.

So why does _extract_reply return empty? Let me check what happens to the AIMessage that agent_node loop=2 produced (the tool-dispatch message with content="" and tool_calls=[book_ticket]). In the cleanup path (is_new_turn=False — no cleanup). So messages contains:

... HumanMessage("can you get passenger from passenger list")
AIMessage(content="I found a saved passenger: ...", tool_calls=[book_ticket]) ← this has content!
ToolMessage(booking result)
AIMessage(content="Your ticket has been successfully booked!") ← loop=3 reply
_extract_reply scans reversed current-turn messages, finds AIMessage("Your ticket has been successfully booked!") → returns it. ✅

So something else is happening. Let me look at _extract_reply in the auto-resume path more carefully — specifically the fact that result["messages"] from ainvoke may not contain the full history. LangGraph's ainvoke returns the state delta from the last node, not the full accumulated state:
...
state.py
messages uses add_messages reducer — so ainvoke returns the full accumulated messages list, not a delta. That's not the issue.

Now I see it. Look at _extract_reply in the auto-resume path:

python

result = await agent_graph.ainvoke(Command(resume=approved), config=config)
reply = _extract_reply(result)
result["messages"] has add_messages reducer — full list. _extract_reply looks for the last HumanMessage. The "yes proceed" was never injected as a HumanMessage into the graph. So last_human_idx points to the last real HumanMessage — "can you get passenger from passenger list" or "yes proceed and book the ticket" depending on whether the initial_state was merged before the interrupt.

Wait — when the graph was first invoked with the "yes proceed" message, initial_state included HumanMessage("yes proceed and book the ticket") and was passed to agent_graph.ainvoke(initial_state, config) → that HumanMessage was added to state before the interrupt. So after resume, the messages list contains that HumanMessage. _extract_reply finds it as last_human_idx, then scans after it.

After that HumanMessage, the sequence is:

AIMessage (confirmation prompt text — from the AI's reply before interrupt: "Just to confirm — I'll book...")
ToolMessage (booking result)
AIMessage (success text)
_extract_reply reversed scan finds the success AIMessage first → returns it. ✅

So the empty reply must be coming from a specific scenario. The LangSmith trace input shows resume: true and the output shows no messages or empty messages. This suggests the issue is that when the "yes proceed" message arrived, session.interrupted was False (not set), so it fell through to _run_graph as a normal turn. Then _run_graph ran the graph with HumanMessage("yes proceed") — and the agent, seeing a message that looks like a continuation but with agent_loop_count=0 (new turn), ran is_new_turn=True, stripped everything, and produced an empty reply because it had no context about what to book.

This is the real root cause: session.interrupted was not set, so "yes proceed" was processed as a normal message instead of a resume. Then _run_graph returned None (interrupt again because pending_tool_calls still there from checkpoint) → showed the confirmation prompt again as a fresh interrupt. The user saw: "Just to confirm..." → booked → success.

Why wasn't session.interrupted set? Because _run_graph sets it via get_session(sid) inside the function. If the session was new (e.g. page reload, reconnect) between the interrupt and the "yes proceed" message, session.interrupted would be False.

But the more reliable fix is: don't rely on in-memory session.interrupted at all. Instead, always check the LangGraph checkpoint state directly — if pending_tool_calls has a destructive tool awaiting confirmation, treat any affirmative reply as a resume.


1 tool call
Command
bash -c "grep -n 'get_state\|aget_state\|pending_tool_calls' /home/tech/Desktop/irctc_agent2/ai-service/app/websocket/manager.py"
404:    # Detect by checking pending_tool_calls for a destructive tool awaiting approval.
408:        pending_calls = result.get("pending_tool_calls") or []
The fix: instead of trusting session.interrupted (in-memory, lost on reconnect), check the LangGraph checkpoint with aget_state to see if there are pending destructive tool calls before deciding whether to resume or run fresh.


1 tool call
Fix unreliable session.interrupted flag by checking LangGraph checkpoint state (aget_state) for pending destructive tool calls before deciding resume vs normal flow. Also fix _extract_reply for resume path where no new HumanMessage is in state.
0/3
Replace session.interrupted check with aget_state checkpoint inspection in on_query_send
NEXT
Fix _extract_reply for resume path: when no HumanMessage exists after last turn, fall back to scanning full current-turn messages
Verify diagnostics clean
Let me re-read the exact section of on_query_send I'm replacing before editing.
...
manager.py
Now I have the exact code. The replacement does two things:

Uses aget_state to check the checkpoint for pending destructive calls — reliable even after reconnect
Passes the user's message as context to the graph on resume so _extract_reply has a proper HumanMessage boundary
...
manager.py
Now fix _extract_reply to handle the resume path where no new HumanMessage boundary exists after the tool result:
...
{"trainNumber": "12925", "travelClass": "SL", "quota": "GN", "journeyDate": "2026-07-25", "status": "AVAILABLE", "count": 37, "label": "AVBL-37", "available": true}
...
{"trainNumber": "12925", "travelClass": "SL", "quota": "GN", "fromStation": "KOTA", "toStation": "NDLS", "distance": 592, "amount": 326, "currency": "INR", "breakdown": {"baseFare": 326, "reservationCharge": 20, "superfastCharge": 30, "gst": 16, "total": 392}}

everything is fine till ehre after this point 

**AI**

No message content

i see in langsmith traces
...
this followed also i noticed in temrinal i saw